...
namespace BoardGameFramework.ConnectFour.Save;

public class ConnectFourGameSave : GameSave
{
    public ConnectFourGameSave(ConnectFourGrid grid, MoveHistory history, Player[] players)
        : base(grid, history, players, "ConnectFour") { }
    ...
    public override void SaveToFile(string path)
    {
        ...
        var data = new
        {
            ...
            DoneStack = History.GetDoneStack()
                .OfType<ConnectFourMoveCommand>()
                .Select(c => new
                {
                    c.Column, c.LandingRow, c.Symbol,
    ...
    public override void LoadFromFile(string path)
    {
        ...
        var commands = root.GetProperty("DoneStack").EnumerateArray()
            .Select(d => (MoveCommand)new ConnectFourMoveCommand(
                ...
                d.GetProperty("LandingRow").GetInt32(),
                d.GetProperty("Symbol").GetString()?[0] ?? ' ')).ToList();
